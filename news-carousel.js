...
(function () {
  ...
  function card(it) {
    ...
    /* Η αγγλική έκδοση σε ξεχωριστό /en/ μόνο όπου υπάρχει (data-en-prefix στην ενότητα) */
    var sec = document.getElementById('newsSection');
    var enPrefix = sec && sec.getAttribute('data-en-prefix');
    var href = isEn() && enPrefix && url.charAt(0) === '/' ? enPrefix + url : url;
  ...
  var _items = null;
  /* Αλλαγή γλώσσας χωρίς νέα φόρτωση σελίδας: ξαναχτίζονται οι κάρτες */
  document.addEventListener('langchange', function () {
    var track = document.getElementById('newsTrack');
    if (!_items || !track) return;
    var one = _items.slice().sort(function (a, b) {
      return String(b.date || '').localeCompare(String(a.date || ''));
    }).map(card).join('');
  ...
  function init() {
    ...
    fetch(base, { cache: 'no-cache' })
      ...
      .then(function (d) {
        if (d && d.items && d.items.length) { _items = d.items; build(d.items); }
        else sec.remove();
      })
